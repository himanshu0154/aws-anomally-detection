 ...
 *
 * The cards are opaque, so the travelling highlight is layered *above* them
 * (`layer="above"`) as a tint that moves from card to card under the pointer; the
 * card's own hover border still marks which one the highlight is on. Each card
 * also names itself to the labelled cursor.
 */
export default function QuickLinks({ badges = {} }: QuickLinksProps) {
  return (
    <nav aria-label="Deeper analysis">
      <AnimatedBackground
        containerClassName="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4"
